...
<xsl:template match="/castxml2pony/ns/record[@render='1']">
  <xsl:variable name="ctype" select="./@ctype"/>

  <xsl:result-document href="../../test/Gtk/{$ctype}.pony">
  <xsl:call-template name="onestruct"><xsl:with-param name="loc" select="/castxml2pony/structs/struct[@name=$ctype]"/></xsl:call-template>
</xsl:result-document>
</xsl:template>
<xsl:template name="onestruct"> <!-- A"/castxml2pony/structs/struct[@id=$struct]"> -->
<xsl:param name="loc"/>
...
/*
  Source: </xsl:text>
    <xsl:variable name="fid" select="$loc/@fid"/>
    <xsl:variable name="originalid" select="$loc/@id"/>
  ...
  Struct Size (bits):  <xsl:value-of select="$loc/@size"/>
  Struct Align (bits): <xsl:value-of select="$loc/@align"/>

  Fields (Offset in bits):
<xsl:apply-templates select="$loc/field" mode="generateCommentField"/>*/
struct <xsl:value-of select="$loc/@name"/><xsl:text>
</xsl:text>

<xsl:if test="$renderfields='1'"><xsl:apply-templates select="$loc/field" mode="generateField"/></xsl:if>
</xsl:template>
