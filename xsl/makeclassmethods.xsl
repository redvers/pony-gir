...
<xsl:template match="/castxml2pony/renders/method[@render='1']">
...
</xsl:text></xsl:if><xsl:text>  fun ref </xsl:text><xsl:value-of select="$n/@ponyname"/>(<xsl:value-of select="$args"/>): <xsl:value-of select="/castxml2pony/typedefs/typedef[@name=$rrv]/@ponytypeout"/> =>
...
<xsl:choose>
	...
	<xsl:when test="((position() = 1) and (@type=concat('NullablePointer[', 'GObject', ']')))"></xsl:when>
	<xsl:when test="((position() = 1) and (@type=$struct))"></xsl:when>
	<xsl:when test="((position() = 1) and (@type='GObject'))"></xsl:when>
	<xsl:otherwise>
